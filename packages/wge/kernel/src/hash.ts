 ...
 *
 * SHA-256 is implemented here directly (FIPS 180-4) so the kernel is
 * isomorphic: the runtime executes wherever a World does — Node or browser
 * (SLI-1500.017 multi-device) — and the synchronous kernel API cannot wait
 * on the async Web Crypto interface. Output is byte-identical to
 * node:crypto's sha256 hex digest.
 */
...
export function hashIndex(index: Record<string, unknown>): string {
  return sha256Hex(serializeCanonicalValue(index));
}
